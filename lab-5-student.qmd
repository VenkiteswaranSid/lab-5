...
person_interview <- inner_join(person, 
                               interview,
                               by = join_by("id" == "person_id"))
nw_interview <- person_interview |>
  filter(address_street_name == "Northwestern Dr") |>
  arrange(desc(address_number)) |>
  slice(1)

annabel_interview <- person_interview |>
  filter(address_street_name == "Franklin Ave",
         name == "Annabel Miller")

nw_interview$transcript
annabel_interview$transcript
...
get_fit_now_member <- inner_join(get_fit_now_member, 
                                 get_fit_now_check_in,
                                 by = join_by("id" == "membership_id"))

person <- inner_join(get_fit_now_member, 
                     person,
                     by = join_by("person_id" == "id"))

person <- inner_join(person,
                     drivers_license,
